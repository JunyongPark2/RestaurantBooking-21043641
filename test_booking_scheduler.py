...
from test_communication import FakeSmsSender, FakeMailSender
...
class FakeBookingScheduler(BookingScheduler):
...
def booking_scheduler_with_sms_mock():
    ...
    testable_sms_sender = FakeSmsSender()
...
def booking_scheduler_with_mail_mock():
    booking_scheduler = BookingScheduler(CAPACITY_PER_HOUR)
    testable_mail_sender = FakeMailSender()
...
def test_시간대별_인원제한이_있다_같은_시간대가_다르면_Capacity_차있어도_스케쥴_추가_성공(booking_scheduler):
    ...
    diffent_hour = ON_THE_HOUR + timedelta(hours=1)
    new_schedule = Schedule(diffent_hour, UNDER_CAPACITY, CUSTOMER)
    booking_scheduler.add_schedule(new_schedule)
...
def test_이메일이_없는_경우에는_이메일_미발송(booking_scheduler_with_mail_mock):
    ...
    assert mail_mock.send_mail_counter == 0
...
def test_이메일이_있는_경우에는_이메일_발송(booking_scheduler_with_mail_mock):
    ...
    assert mail_mock.send_mail_counter == 1


def test_현재날짜가_일요일인_경우_예약불가_예외처리():
    booking_scheduler = FakeBookingScheduler(CAPACITY_PER_HOUR, '2021/03/28 17:00')
    schedule = Schedule(ON_THE_HOUR, UNDER_CAPACITY, CUSTOMER)
...
def test_현재날짜가_일요일이_아닌경우_예약가능():
    booking_scheduler = FakeBookingScheduler(CAPACITY_PER_HOUR, '2024/06/03 17:00')
    schedule = Schedule(ON_THE_HOUR, UNDER_CAPACITY, CUSTOMER)
